...
double StepsPermmZ = 80.0 / 5.0;
long zHubUp = 0, zHubDown = -100;
int DownPause = 200000;

int fd = 0;
struct termios TermOpt;

int cncMode = 0; //0 - Engrave / plot
                 //1 - Plastic
                 //2 - Wood
                 //3 - Aluminum
int relayOn = 0;
...
void PrintMenue_01(){
  ...
  char Text[300];
   ...
   clrscr(1, MessageY-2);
   MessageText("*** Main menu CNC ***", 1, 1, 1);
   ...
   sprintf(TextLine, "- / +        - decrease or increase lowest point of miller for around 0.5mm");
   ...
   if(cncMode == 3){
     strcpy(Text, "Aluminium");
   }
   sprintf(TextLine, "C            - toggle CNC mode: \"%s\"", Text);
   MessageText(TextLine, 10, 18, 0);

   MessageText("Esc          - leave program", 10, 19, 0);
...
void PrintMenue_03(char *FullFileName, long CurrentLine, long NumberOfLines, long CurrentX, long CurrentY, long StartTime){
  ...
  long CurrentTime, ProcessHours = 0, ProcessMinutes = 0, ProcessSeconds = 0;
   ...
   clrscr(1, MessageY - 2);
   MessageText("*** Processing file ***", 1, 1, 1);
   ...
   sprintf(TextLine, "Number of steps: X=%ld, Y=%ld", stepCountX, stepCountY);
...
int main(int argc, char **argv){
  ...
  while (1){
    ...
    if(MenueLevel == 0){
      ...
      if(KeyHit == 't'){//Testing
        CalculatePlotter(0, 400, stepPause, 0);
...
/*      
        if(cncMode == 1){
        for(i=zHubDown + 15; i > zHubDown; i-=5){
          moveZ(zHubUp, stopPlot);
          usleep(AdvanceRate * 1000);
          moveZ(i, stopPlot);
          usleep(AdvanceRate * 1500);
        }
        }
        if(cncMode == 2){
        for(i=zHubUp; i > zHubDown; i-=1){
          moveZ(i, stopPlot);
          usleep(AdvanceRate * 500);
        }
        }
        moveZ(zHubDown, stopPlot);
*/
        ...
        if(strcmp(FileName, "noFiLE") != 0 && KeyHit != 27){
          ...
          while(!(feof(PlotFile)) && stopPlot == 0){
            ...
            if(ReadState == 6){//Y value
              ...
              //doubleTemp = yMax - strtol(TextLine, &pEnd, 10);//Mirror Y axis
